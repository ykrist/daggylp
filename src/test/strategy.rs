...
pub trait SharableStrategy:
  ProptestStrategy<
    Value = <Self as SharableStrategy>::Value,
    Tree = <Self as SharableStrategy>::Tree,
  > + Clone
  + Send
  + Sync
  + 'static
...
pub fn nodes(
  lb: impl SharableStrategy<Value = Weight>,
  ub: impl SharableStrategy<Value = Weight>,
  obj: impl SharableStrategy<Value = Weight>,
) -> impl SharableStrategy<Value = NodeData> {
...
pub fn any_bounds_nodes(
  size: impl Into<SizeRange>,
) -> impl SharableStrategy<Value = Vec<NodeData>> {
  prop::collection::vec(
...
pub fn any_edge_weight() -> impl SharableStrategy<Value = Weight> {
  0..MAX_EDGE_WEIGHT
}
...
pub fn graph_with_conn(
  ...
  nodes
    ...
    .prop_map(|(nodes, (edges, edge_weights))| GraphData {
      nodes,
  ...
  nodes
    ...
    .prop_map(|(nodes, edges)| {
      let edges = edges
        ...
        .filter_map(|(mut w, (i, j))| w.map(|w| ((i, j), w)))
        .collect();
      GraphData { edges, nodes }
    })
}
...
// // }
//
pub fn complete_graph_zero_edges(
  nodes: impl SharableStrategy<Value = Vec<NodeData>>,
) -> impl SharableStrategy<Value = GraphData> {
  graph_with_conn(nodes, AllEdges(), Just(0))
}

pub fn complete_graph_nonzero_edges(
  nodes: impl SharableStrategy<Value = Vec<NodeData>>,
) -> impl SharableStrategy<Value = GraphData> {
...
pub fn scc_graph_conn(size: usize) -> impl SharableStrategy<Value = SccGraphConn> {
  let tri = if Triangular::strongly_connected(size) {
    1
  } else {
    0
  };
  let sq = if Square::strongly_connected(size) {
    1
  } else {
    0
  };
...
pub fn scc_graph(
  ...
  let nodes = match feas {
    SccKind::Feasible | SccKind::InfEdge => nodes(MIN_WEIGHT..=0, 0..=MAX_WEIGHT, Just(0)),
    SccKind::InfBound => nodes(MIN_WEIGHT..=MAX_WEIGHT, MIN_WEIGHT..=MAX_WEIGHT, Just(0)),
  };
  let nodes = prop::collection::vec(nodes, size);
  let edge_weights = match feas {
    SccKind::Feasible | SccKind::InfBound => 0..=0,
    SccKind::InfEdge => 0..=MAX_EDGE_WEIGHT,
  };

  nodes
    .prop_ind_flat_map2(|nodes| scc_graph_conn(nodes.len()))
    .prop_flat_map(move |(nodes, conn)| graph_with_conn(Just(nodes), conn, edge_weights.clone()))
    .prop_map(move |mut g| {
      match feas {
        SccKind::InfEdge => {
...
pub fn set_arbitrary_edge_to_one(
  graph: impl SharableStrategy<Value = GraphData>,
) -> impl SharableStrategy<Value = GraphData> {
  graph
    .prop_ind_flat_map(|graph| {
