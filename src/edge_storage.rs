...
pub trait EdgeLookup: Clone + Neighbours<ForwardDir> + Neighbours<BackwardDir> {
  ...
  fn successor_nodes(
    &self,
    node: usize,
  ) -> EdgesToNodes<<Self as Neighbours<ForwardDir>>::Neigh<'_>> {
  ...
  fn predecessor_nodes(
    &self,
    node: usize,
  ) -> EdgesToNodes<<Self as Neighbours<BackwardDir>>::Neigh<'_>> {
  ...
  fn neighbour_nodes<D: EdgeDir>(
    &self,
    node: usize,
  ) -> EdgesToNodes<<Self as Neighbours<D>>::Neigh<'_>>
  where
    Self: Neighbours<D>,
  {
...
type CsrAllEdges<'a, V> =
  iter::FlatMap<std::slice::Iter<'a, V>, &'a [Edge], fn(&'a V) -> &'a [Edge]>;
...
impl<V: EdgeList<Edge>> EdgeLookup for AdjacencyList<V> {
    ...
    if let Some(mut should_remove) = should_remove {
      for edge_lookup in &mut [edges_from, edges_to] {
        for edgelist in edge_lookup.iter_mut() {
          edgelist.retain(|e| !(should_remove(e) || edge_remove_queue.contains(&(e.from, e.to))))
        }
      }
    } else {
      for edge_lookup in &mut [edges_from, edges_to] {
  ...
  fn add_new_component(
    &mut self,
    num_new_nodes: usize,
    new_edges: FnvHashMap<(usize, usize), Edge>,
  ) {
...
mod csr_alist {
  ...
  unsafe fn sort_and_build_start_len_vec(
    dest: &mut Vec<(usize, usize)>,
    entries: &mut [Option<Edge>],
    num_new_nodes: usize,
    idx_node: impl Fn(&Edge) -> usize,
  ) {
    entries.sort_unstable_by_key(|entry| idx_node(entry.as_ref().unwrap_unchecked()));
    let mut sorted_entries = entries
      .iter()
      .map(|e| idx_node(e.as_ref().unwrap_unchecked()));
  ...
  impl BuildEdgeStorage for CsrAdjacencyListBuilder {
    ...
    fn new(num_nodes: usize) -> Self {
      ...
      CsrAdjacencyListBuilder {
        num_nodes,
        edges: Vec::with_capacity(2 * num_nodes),
      }
    }
    ...
    fn finish(self) -> CsrAdjacencyList {
      ...
      unsafe {
        sort_and_build_start_len_vec(
          &mut edges_from_start_and_len,
          &mut edges_from,
          num_nodes,
          |e| e.from,
        )
      ...
      // Safety: we never add None entries to `edges` in the builder
      unsafe {
        sort_and_build_start_len_vec(&mut edges_to_start_and_len, &mut edges_to, num_nodes, |e| {
          e.to
        })
      };
  ...
  impl EdgeLookup for CsrAdjacencyList {
    ...
    fn add_new_component(
      ...
      unsafe {
        sort_and_build_start_len_vec(
          &mut self.edges_from_start_and_len,
      ...
      self
        .edges_to
        .extend_from_slice(&self.edges_from[component_start..]);
      self.edges_to_start_and_len.reserve(num_new_nodes);
      // Safety: the entries we just added from `start` onwards are all `Some(..)`.
      unsafe {
        sort_and_build_start_len_vec(
          &mut self.edges_to_start_and_len,
      ...
      debug_assert_eq!(
        self.edges_to_start_and_len.len(),
        self.edges_from_start_and_len.len()
      );
    }
