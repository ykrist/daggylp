...
impl Iis {
  ...
  ///
  ///  `(a <- b), (b <- c) ,...,(y <- z), (z <- a)`
  pub(crate) fn from_backwards_cycle<E>(
    graph: &Graph<E>,
    // bounds: Option<(usize, usize)>,
    nodes: impl IntoIterator<Item = usize>,
  ) -> Self {
  ...
  /// where `a` is the UB violated node and `z` is the LB violated node.
  ///
  pub(crate) fn from_cycle_path_pair<E>(
    graph: &Graph<E>,
    backwards_path: impl IntoIterator<Item = usize>,
    forwards_path: impl IntoIterator<Item = usize>,
  ) -> Self {
    let mut backwards_path = backwards_path.into_iter();
    let mut forwards_path = forwards_path.into_iter();

    let mut edge_nodes =
      Vec::with_capacity(backwards_path.size_hint().0 + forwards_path.size_hint().0 - 1);
  ...
  pub(crate) fn from_backwards_path<E>(
    graph: &Graph<E>,
    nodes: impl IntoIterator<Item = usize>,
  ) -> Self {
  ...
  pub fn bounds(&self) -> Option<(Var, Var)> {
    self
      .bounds
      .map(|(lb, ub)| (self.var_from_node_id(lb), self.var_from_node_id(ub)))
  }
  ...
  pub fn compute_iis(&mut self, minimal: bool) -> Iis {
    ...
    #[cfg(feature = "viz-extra")]
    {
      ...
      self.viz_data.highlighted_edges.extend(
        iis
          .iter_edge_constraints()
          .map(|(vi, vj)| (vi.node, vj.node)),
      );
      self
        .viz_data
        .highlighted_nodes
        .extend(iis.iter_bounds().map(|c| match c {
